...
>
	<div class="flex items-center gap-4">
		...
		<div
			class="bg-neutral-800 p-2 rounded-full text-orange-500"
			class:hover:bg-orange-500={!isDragging}
			class:hover:text-white={!isDragging}
		>
			<svg
		...
		<div class="h-fit" class:hidden={$tabletMode}>
			<label
				for="CameraPresetName"
				class="relative block border-b bg-transparent focus-within:border-orange-500 border-neutral-700"
			>
				<input
					type="text"
					id="CameraPresetName"
		...
		<div
			class="bg-neutral-800 p-2 rounded-full h-fit text-orange-500"
			class:hover:bg-orange-500={!isDragging}
			class:hover:text-white={!isDragging}
			role="button"
			tabindex="0"
			on:click|preventDefault={toggleSettings}
			on:keydown|preventDefault={(e) => e.key === 'Enter' && toggleSettings()}
			class:hidden={$tabletMode}
		>
			<svg
				xmlns="http://www.w3.org/2000/svg"
				fill="none"
		...
		<!-- Button for dragging -->
		<div
			class="bg-neutral-800 p-2 rounded-full text-orange-500 hover:bg-orange-500 hover:text-white handle-{cameraPreset.id}"
			class:bg-orange-500={isDragging}
		...
		<form on:submit|preventDefault={handleSubmit} class="mt-2">
			<div class="flex">
				...
				<div class="w-full flex flex-col items-center text-xs">
					...
					<div class="flex items-center rounded border border-gray-800">
						<button
							type="button"
							class="size-6 leading-6 transition hover:text-orange-500 text-neutral-400"
							on:click|preventDefault={() =>
								(cameraPreset.camera_settings.position.x = Math.max(
									0,
									cameraPreset.camera_settings.position.x - 1
								))}
						>
							&minus;
						</button>
						...
						<input
							type="number"
							id="camera_setting_preset_position_x"
				...
				<div class="w-full flex flex-col items-center text-xs">
					...
					<div class="flex items-center rounded border border-gray-800">
						<button
							...
							class="size-6 leading-6 transition hover:text-orange-500 text-neutral-400"
							on:click|preventDefault={() =>
								(cameraPreset.camera_settings.position.y = Math.max(
									0,
									cameraPreset.camera_settings.position.y - 1
								))}
						>
							&minus;
						</button>
						...
						<input
							type="number"
							id="camera_setting_preset_position_y"
							class="focus:border-orange-500 focus:outline-none focus:ring-0 h-10 w-14 border-transparent text-center [-moz-appearance:_textfield] sm:text-sm bg-neutral-800 text-white [&::-webkit-inner-spin-button]:m-0 [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:m-0 [&::-webkit-outer-spin-button]:appearance-none"
				...
				<div class="w-full flex flex-col items-center text-xs">
					...
					<div class="flex items-center rounded border border-gray-800">
						<button
							...
							class="size-6 leading-6 transition hover:text-orange-500 text-neutral-400"
							on:click|preventDefault={() =>
								(cameraPreset.camera_settings.zoom = Math.max(
									0,
									cameraPreset.camera_settings.zoom - 1
								))}
						>
							&minus;
						</button>
			...
			<label for="cameraSelect" class="w-full flex flex-col items-center text-xs">
				...
				<select
					...
					name="cameraSelect"
					id="cameraSelect"
					class="mt-0.5 w-full rounded shadow-sm sm:text-sm border-gray-800 bg-neutral-800 text-white focus:border-orange-500 focus:outline-none focus:ring-0 h-10"
				>
			...
			<div class="flex items-center mt-4 justify-center gap-4">
				<button
					type="submit"
					class="bg-green-700 text-white rounded p-3 hover:bg-green-600"
				...
				<button
					type="button"
					class="bg-red-700 text-white rounded p-3 hover:bg-red-600"
					aria-label="Delete Button"
					on:click|preventDefault={handeDelete}
				>
					<svg
						xmlns="http://www.w3.org/2000/svg"
						fill="none"
